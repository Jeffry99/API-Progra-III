...
public class TramiteEstadoController {
    ...
    @PreAuthorize("hasAuthority('TRU01')")
     public ResponseEntity<?> create(@RequestBody TramiteEstadoDTO tramiteEstado) {
        try {
            return new ResponseEntity<>(tramiteEstadoService.create(tramiteEstado), HttpStatus.CREATED);
        }catch(Exception ex){
            return new ResponseEntity<>(ex, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @ Valid @RequestBody TramiteEstadoDTO tramiteEstado, BindingResult bindingResult) {
        if(!bindingResult.hasErrors()){
            try{
                Optional<TramiteEstadoDTO> updated = tramiteEstadoService.update(tramiteEstado, id);
